...
bus.on("order:paid", (order) => {
    ...
    bus.emit("order:statusChanged", { id: order.id, status: "paid" });
});
...
bus.on("order:packed", (order) => {
    ...
    bus.emit("order:statusChanged", { id: order.id, status: "packed" });
});
...
bus.on("order:shipped", (order) => {
    ...
    bus.emit("order:statusChanged", { id: order.id, status: "shipped" });
});
...
bus.on("order:canceled", (order) => {
    ...
    bus.emit("order:statusChanged", { id: order.id, status: "canceled" });
});
